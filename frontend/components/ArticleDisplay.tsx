...
export default function ArticleDisplay({ article, onRegenerateRequest }: ArticleDisplayProps) {
  ...
  const downloadHTML = () => {
    ...
    const processedContent = article.content.replace(/!\{\{([^}]+)\}\}\(placeholder\)/g, (match, imageId) => {
...
`.trim();
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'jenosize-article.txt';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="space-y-6">
      ...
      {article.sourceUsageDetails && article.sourceUsageDetails.length > 0 && (
        <div className="card">
          <div className="flex items-center space-x-4 mb-8">
            <div className="icon-wrapper w-16 h-16">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
